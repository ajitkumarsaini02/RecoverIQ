...
export const Transactions: React.FC = () => {
  ...
  const getStatusBadge = (status: TransactionStatus) => {
    switch (status) {
      ...
      case 'FAILED':
        return (
          <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[11px] font-mono font-medium bg-rose-500/10 text-rose-400 border border-rose-500/30">
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-xs text-slate-400 mt-1">
            Complete transaction ledger with customer profiles, failure diagnostics, AI actions, and risk evaluations.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-surface-card border border-surface-border text-xs font-mono text-slate-300 hover:text-white hover:border-slate-600 transition-colors flex items-center gap-1.5 cursor-pointer"
      ...
      <div className="p-4 rounded-xl glass-card border border-surface-border grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="flex items-center justify-between gap-2">
          ...
          <select
            ...
            className="px-3 py-2 rounded-lg bg-surface-base border border-surface-border text-xs text-slate-200 focus:outline-none focus:border-brand-500 cursor-pointer"
          >
          ...
          <button
            onClick={loadData}
            disabled={loading}
            className="p-2 rounded-lg bg-surface-base border border-surface-border text-slate-300 hover:text-white cursor-pointer"
            title="Refresh"
      ...
      <div className="rounded-xl glass-panel border border-surface-border overflow-hidden shadow-enterprise">
              ...
              {loading ? (
                Array.from({ length: 8 }).map((_, idx) => (
                  <tr key={idx} className="animate-pulse">
                    ...
                    <td className="py-3 px-4"><div className="h-4 bg-slate-800 rounded w-20" /></td>
                    <td className="py-3 px-4"><div className="h-4 bg-slate-800 rounded w-14" /></td>
                    <td className="py-3 px-4"><div className="h-4 bg-slate-800 rounded w-20" /></td>
                    <td className="py-3 px-4 text-right"><div className="h-4 bg-slate-800 rounded w-10 ml-auto" /></td>
                  </tr>
                ))
              ) : transactions.length === 0 ? (
                <tr>
                  <td colSpan={9} className="py-12 text-center text-slate-400 space-y-2">
                    <AlertCircle className="h-8 w-8 mx-auto text-slate-500 mb-1" />
                    <p className="font-medium text-slate-300">No transactions found matching filters.</p>
                    <p className="text-xs text-slate-500">Try adjusting your search criteria or resetting filters.</p>
                  </td>
                </tr>
              ) : (
                transactions.map((txn) => {
                  const latestAction = txn.recovery_actions?.[0];
                  const aiAction = latestAction?.action_type || (txn.status === 'RECOVERED' ? 'RETRY_PAYMENT' : txn.status === 'STOPPED' ? 'STOP' : 'PENDING_ANALYSIS');
                  const riskLevel = latestAction?.ai_risk_level || (txn.status === 'STOPPED' ? 'HIGH' : 'LOW');
                    ...
                    >
                      ...
                      <td className="py-3 px-4">
                        <div className="font-medium text-slate-200 truncate max-w-[150px]">
                          {txn.customer_name || txn.customer?.name || 'Customer'}
                        </div>
                        <div className="text-[11px] font-mono text-slate-400 truncate max-w-[150px]">
                          {txn.customer_email || txn.customer?.email}
                        </div>
                      </td>
                      ...
                      <td className="py-3 px-4">
                        <span className="text-[11px] font-mono px-2 py-0.5 rounded bg-purple-500/15 text-purple-300 border border-purple-500/30 font-semibold truncate block max-w-[160px]">
                          {aiAction}
                        </span>
                      </td>
                      <td className="py-3 px-4">
                        <span className={`text-[10px] font-mono font-bold px-2 py-0.5 rounded border ${
                          riskLevel === 'HIGH' ? 'bg-rose-500/15 text-rose-300 border-rose-500/30' : 'bg-emerald-500/15 text-emerald-300 border-emerald-500/30'
                        }`}>
                          {riskLevel}
                        </span>
                      </td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <button
                          ...
                          title="View Context & Diagnostics"
        ...
        {/* Pagination Footer */}
        <div className="px-6 py-3.5 border-t border-surface-border bg-surface-card/60 flex flex-col sm:flex-row items-center justify-between gap-3 text-xs text-slate-400">
          <div>
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="p-1.5 rounded-lg bg-surface-base border border-surface-border text-slate-300 disabled:opacity-40 disabled:cursor-not-allowed hover:text-white hover:border-slate-500 cursor-pointer"
